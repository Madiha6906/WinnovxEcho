...
async function recordAudio(language: string, ms: number): Promise<string> {
  ...
  return new Promise((resolve) => {
    ...
    mediaRecorder.onstop = async () => {
      ...
      try {
        const res = await fetch(`http://127.0.0.1:8000/transcribe?lang=${encodeURIComponent(language)}`,
        {
          method: "POST",
          body: formData,
        });
...
function doSpeak(text: string, resolve: () => void) {
  speak(text).then(() => setTimeout(() => resolve(), 800));
}
...
export default function ProductivityMode({ onBack }: ProductivityModeProps) {
  ...
  async function startFlow() {
    ...
    await speakAndWait(getMessage('productivityMode', 'askTasks', language));

    setStatus("Listening... Speak all your tasks now");
    const text = await recordAudio(speechLanguage[language], 20000);
    setTranscript(text);

    if (!text || text.trim().length < 5) {
      setStatus(getMessage('productivityMode', 'couldNotHear', language));
      await speakAndWait(getMessage('productivityMode', 'couldNotHear', language));
      return startFlow();
    }
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white px-6">
        <div className="max-w-2xl w-full flex flex-col items-center gap-8">
          <h1 className="text-[48px] font-bold text-center" style={{ fontFamily: "Neuton, serif" }}>
            {getMessage('productivityMode', 'title', language)}
          </h1>
          <div className="bg-[#1a1a1a] border-2 border-[#FFD700] rounded-lg p-10 w-full flex flex-col items-center gap-6">
            ...
            <p className="text-[22px] text-center" style={{ fontFamily: "Inter, sans-serif" }}>
              ...
              {status.startsWith("Could") && getMessage('productivityMode', 'couldNotHear', language)}
        ...
        <div className="max-w-4xl mx-auto flex flex-col gap-6">
          ...
          <div className="bg-green-900 border-2 border-green-400 rounded-xl p-8 text-center">
            <p className="text-[32px] font-bold text-green-400" style={{ fontFamily: "Neuton, serif" }}>
              {getMessage('productivityMode', 'allTasksCompleted', language)}
            </p>
            ...
            <div className="space-y-3">
              {tasks.map((task) => (
                <div key={task.id} className="flex justify-between items-center border border-gray-700 rounded-lg p-4">
                  <div>
                    <p className="font-bold text-[#FFD700]" style={{ fontFamily: "Neuton, serif" }}>{getMessage('productivityMode', 'taskOfCount', language, { id: String(task.id), total: String(tasks.length) })}</p>
                    <p
                    ...
                    <p className="text-gray-400 text-[14px]">{getMessage('productivityMode', 'taskTimeSpent', language)} {formatTimeSpeak(task.timeSpent)}</p>
                  </div>
                  <div>
    ...
    <div className="min-h-screen bg-black text-white px-6 py-8">
      <div className="max-w-4xl mx-auto flex flex-col gap-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-[#FFD700] text-[18px]" style={{ fontFamily: "Inter, sans-serif" }}>
              {getMessage('productivityMode', 'voiceAssistantActive', language)}
            </p>
          </div>
          <button
            onClick={() => { stopListening(); onBack(); }}
        ...
        <div className="bg-[#1a1a1a] border border-[#FFD700] rounded-lg p-4 flex items-center gap-4">
          ...
          />
          <div>
            <p className="text-[18px]" style={{ fontFamily: "Inter, sans-serif" }}>{getMessage('productivityMode', 'statusLabel', language)} {status}</p>
            {transcript && (
              <p className="text-gray-400 text-[13px]">
                {getMessage('modeSelection', 'heard', language)} "{transcript}"
              </p>
            )}
            ...
            <div className="flex flex-col items-center gap-2 mb-6">
              ...
              <p className="text-gray-400 text-[16px]">
                {currentTask.duration > 0 ? getMessage('productivityMode', 'countdown', language) : getMessage('productivityMode', 'stopwatch', language)} —{" "}
                <span className={timerRunning ? "text-green-400" : "text-red-400"}>
                  {timerRunning ? `🟢 ${getMessage('productivityMode', 'running', language)}` : `🔴 ${getMessage('productivityMode', 'stopped', language)}`}
                </span>
              </p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                style={{ fontFamily: "Neuton, serif" }}
              >
                <CheckCircle size={24} /> {getMessage('productivityMode', 'done', language)}
              </button>
              <button
                ...
                style={{ fontFamily: "Neuton, serif" }}
              >
                <XCircle size={24} /> {getMessage('productivityMode', 'notDone', language)}
              </button>
            </div>
          </div>
        </div>
