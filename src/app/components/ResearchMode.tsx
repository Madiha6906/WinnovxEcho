...
function speakAndWait(text: string): Promise<void> {
  ...
  return sentences.reduce((chain, sentence, index) => {
    return chain.then(() => {
      return speak(sentence).then(() => {
...
function speakNowText(text: string) {
  speechSynthesis.cancel();
  speakNow(text);
}

async function recordAudio(ms: number, language: string): Promise<string> {
  try {
...
export default function ResearchMode({ onBack }: ResearchModeProps) {
  ...
  const [status, setStatus] = useState(getMessage('researchMode', 'statusIdle', language));
  ...
  function getSectionLabel(section: string) {
    const sectionKey = (`section${section.charAt(0).toUpperCase()}${section.slice(1)}`) as
      | 'sectionAbstract'
      | 'sectionIntroduction'
      | 'sectionConclusion';
    return getMessage('researchMode', sectionKey, language);
  }
  ...
  async function startFlow() {
    ...
    setScreen("intro");
    setStatus(getMessage('researchMode', 'statusSpeaking', language));
    stopListening();
    speechSynthesis.cancel();

    await speakAndWait(
      getMessage('researchMode', 'welcome', language)
    );

    // try listening up to two times before giving up
    for (let attempt = 0; attempt < 2; attempt++) {
      setScreen("listening");
      setStatus(getMessage('researchMode', 'statusListening', language));

      // small pause to ensure TTS has fully stopped and mic can start
      try { speechSynthesis.cancel(); } catch (_) {}
      await new Promise(r => setTimeout(r, 400));

      const text = await listenOnce(10000, language);
      setTranscript(text);
      ...
      if (topicAttemptsRef.current >= 2) {
        await speakAndWait(getMessage('researchMode', 'couldNotUnderstandTopic', language));
        setScreen("intro");
        setStatus(getMessage('researchMode', 'statusIdle', language));
        return;
      }

      await speakAndWait(getMessage('researchMode', 'sayTopicAgain', language));
    }
  }
  ...
  async function doSearch(query: string) {
    setScreen("searching");
    setStatus(getMessage('researchMode', 'statusSearching', language));

    await speakAndWait(getMessage('researchMode', 'searchingFor', language, { query }));

    const results = await fetchPapersFromBackend(query);
    console.log("Search results:", results);

    if (!results || results.length === 0) {
      await speakAndWait(getMessage('researchMode', 'noPapersFound', language, { query }));
      return startFlow();
    }
    ...
    setPapers(results);
    setScreen("papers");
    setStatus(getMessage('researchMode', 'readyStatus', language));

    await speakAndWait(getMessage('researchMode', 'foundPapers', language, {
    ...
    for (let i = 0; i < results.length; i++) {
      await speakAndWait(
        getMessage('researchMode', 'paperSummary', language, {
          index: (i + 1).toString(),
    ...
    await speakAndWait(getMessage('researchMode', 'readPaperPrompt', language));
  ...
  async function readPaper(paper: Paper) {
    ...
    await speakAndWait(
      getMessage('researchMode', 'selectedPaper', language, {
    ...
    await speakAndWait(getMessage('researchMode', 'readingInstructions', language));

    startHighlightListener();

    const sections = [
      { key: "abstract", label: getSectionLabel("abstract"), text: paper.abstract },
      { key: "introduction", label: getSectionLabel("introduction"), text: paper.introduction },
      { key: "conclusion", label: getSectionLabel("conclusion"), text: paper.conclusion },
    ];
    ...
    for (const section of allSections) {
      ...
      for (let i = 0; i < section.sentences.length; i++) {
        ...
        setStatus(getMessage('researchMode', 'statusWaitingForHighlight', language));
        ...
        setStatus(getMessage('researchMode', 'statusReading', language));
      }
    }
    ...
    if (!stopReadingRef.current) {
      ...
      const answer = await askYesNo(
        getMessage('researchMode', 'finishedReadingHighlights', language, { count: count.toString() }),
        language
      );
      ...
      if (answer === true) {
        ...
        await speakAndWait(getMessage('researchMode', 'goingBackToPapers', language));
        setScreen("papers");
        startVoiceListener();
      } else {
        await speakAndWait(getMessage('researchMode', 'didNotHearYesNo', language));
        setScreen("papers");
        startVoiceListener();
      }
    }
  ...
  async function readHighlights() {
    ...
    if (h.length === 0) {
      await speakAndWait(getMessage('researchMode', 'noHighlights', language));
      setScreen("papers");
      startVoiceListener();
      return;
    }
    ...
    await speakAndWait(
      getMessage('researchMode', 'savedHighlightsSummary', language, { count: h.length.toString() })
    );

    for (let i = 0; i < h.length; i++) {
      await speakAndWait(
        getMessage('researchMode', 'highlightEntry', language, {
          number: (i + 1).toString(),
          section: h[i].section,
          text: h[i].text,
        })
    ...
    await speakAndWait(getMessage('researchMode', 'allHighlightsRead', language));
  ...
  function startHighlightListener() {
    ...
    rec.lang = speechLanguage[language];
    ...
    rec.onresult = (event: any) => {
      ...
      if (
        ...
        cmd.includes("హైలైట్") ||
        cmd.includes("సేవ్") ||
        cmd.includes("సేవ్ చేయండి") ||
        cmd.includes("ఇది నాకు నచ్చింది")
      ) {
        ...
        if (highlightWindowRef.current && !highlightCapturedRef.current && sentence && sentence.trim().length > 3 && paper) {
          ...
          if (!already) {
            ...
            speakNow(getMessage('researchMode', 'savedToHighlights', language));

            if (highlightResolveRef.current) {
              highlightResolveRef.current();
            }
          } else {
            speakNow(getMessage('researchMode', 'alreadyHighlighted', language));
          }
        }
        return;
      }

      if (cmd.includes("stop reading") || cmd.includes("చదవడం ఆపు") || cmd.includes("ఆపు") || (isFinal && cmd.includes("stop"))) {
        stopReadingRef.current = true;
        speechSynthesis.cancel();
        speakNow(getMessage('researchMode', 'readingStopped', language));
        return;
      }
  ...
  function handleNavCommand(cmd: string) {
    ...
    if (cmd.includes("new topic") || cmd.includes("search again") || cmd.includes("search new") || cmd.includes("another topic") || cmd.includes("కొత్త విషయం") || cmd.includes("మరొక విషయం")) {
      stopListening(); speechSynthesis.cancel(); startFlow(); return;
    }

    if (cmd.includes("highlights") || cmd.includes("my highlights") || cmd.includes("hear highlights") || cmd.includes("హైలైట్")) {
      stopListening(); readHighlights(); return;
    }

    const p1 = cmd.includes("paper one") || cmd.includes("paper 1") || cmd.includes("read one") || cmd.includes("first paper") || cmd.includes("paper number one") || cmd.includes("పత్రం ఒకటి") || cmd.includes("ఒకటి") || cmd.includes("మొదటి పత్రం");
    const p2 = cmd.includes("paper two") || cmd.includes("paper 2") || cmd.includes("read two") || cmd.includes("second paper") || cmd.includes("paper number two") || cmd.includes("పత్రం రెండు") || cmd.includes("రెండవ పత్రం") || cmd.includes("రెండు");
    ...
    if (p2) {
      ...
      if (paper) { stopListening(); readPaper(paper); }
      else speakNow(getMessage('researchMode', 'paperUnavailable', language));
      return;
    }
  }
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white px-6">
        <div className="max-w-2xl w-full flex flex-col items-center gap-8">
          <h1 className="text-[48px] font-bold text-center" style={{ fontFamily: "Neuton, serif" }}>
            {getMessage('researchMode', 'title', language)}
          </h1>
          <div className="bg-[#1a1a1a] border-2 border-[#FFD700] rounded-lg p-10 w-full flex flex-col items-center gap-6">
            ...
            <p className="text-[22px] text-center" style={{ fontFamily: "Inter, sans-serif" }}>
              {getMessage('researchMode', 'voiceAssistantSpeaking', language)}
            </p>
            <p className="text-gray-400 text-[16px] text-center">{getMessage('researchMode', 'microphoneOpensAfterQuestion', language)}</p>
          </div>
          <button onClick={() => { stopListening(); speechSynthesis.cancel(); onBack(); }}
            ...
            <ArrowLeft size={20} /> {getMessage('researchMode', 'back', language)}
          </button>
        </div>
      </div>
    );
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white px-6">
        <div className="max-w-2xl w-full flex flex-col items-center gap-8">
          ...
          <div className="bg-[#1a1a1a] border-2 border-[#FFD700] rounded-lg p-10 w-full flex flex-col items-center gap-6">
            ...
            <p className="text-[22px] text-center" style={{ fontFamily: "Inter, sans-serif" }}>
              {getMessage('researchMode', 'speakTopicNow', language)}
            </p>
            <p className="text-gray-400 text-[16px] text-center">{getMessage('researchMode', 'secondsToSpeak', language)}</p>
            <span className="text-[#FFD700] text-[18px]">{getMessage('researchMode', 'statusLabel', language)} {status}</span>
          </div>
          {transcript && <p className="text-gray-400 text-[14px] text-center">{getMessage('researchMode', 'heard', language)} "{transcript}"</p>}
          <button onClick={() => { stopListening(); speechSynthesis.cancel(); onBack(); }}
            className="w-full min-h-[55px] border-2 border-[#FFD700] text-[#FFD700] rounded-lg flex items-center justify-center gap-3 hover:bg-[#FFD700] hover:text-black transition-colors font-bold"
            style={{ fontFamily: "Neuton, serif" }}>
            <ArrowLeft size={20} /> {getMessage('researchMode', 'back', language)}
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white px-6">
        <div className="max-w-2xl w-full flex flex-col items-center gap-8">
          ...
          <div className="bg-[#1a1a1a] border-2 border-[#FFD700] rounded-lg p-10 w-full flex flex-col items-center gap-6">
            ...
            <p className="text-[22px] text-center" style={{ fontFamily: "Inter, sans-serif" }}>
              {getMessage('researchMode', 'searchingPapers', language)}
            </p>
            <p className="text-[#FFD700] text-[18px]">{getMessage('researchMode', 'topicLabel', language)} {topic}</p>
          </div>
          <button onClick={() => { stopListening(); speechSynthesis.cancel(); onBack(); }}
            className="w-full min-h-[55px] border-2 border-[#FFD700] text-[#FFD700] rounded-lg flex items-center justify-center gap-3 hover:bg-[#FFD700] hover:text-black transition-colors font-bold"
            style={{ fontFamily: "Neuton, serif" }}>
      ...
      <div className="min-h-screen bg-black text-white px-6 py-8">
        <div className="max-w-4xl mx-auto flex flex-col gap-6">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-[48px] font-bold" style={{ fontFamily: "Neuton, serif" }}>{getMessage('researchMode', 'title', language)}</h1>
              <p className="text-[#FFD700] text-[18px]">{getMessage('researchMode', 'topicLabel', language)} {topic}</p>
            </div>
            <button onClick={() => { stopListening(); speechSynthesis.cancel(); onBack(); }}
          ...
          <div className="bg-[#1a1a1a] border border-[#FFD700] rounded-lg p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-[16px]" style={{ fontFamily: "Inter, sans-serif" }}>{getMessage('researchMode', 'statusLabel', language)} {status}</p>
              {transcript && <p className="text-gray-400 text-[13px]">{getMessage('researchMode', 'heard', language)} "{transcript}"</p>}
            </div>
          </div>
            ...
            {papers.map((paper, i) => (
              <div key={paper.id} className="bg-[#1a1a1a] rounded-xl p-6 border border-gray-700">
                ...
                <button onClick={() => { stopListening(); readPaper(paper); }}
                  ...
                  style={{ fontFamily: "Neuton, serif" }}>
                  <Play size={22} /> {getMessage('researchMode', 'readPaperButton', language, { index: (i + 1).toString() })}
                </button>
              </div>
            ))}
          </div>
          ...
          <button onClick={() => { stopListening(); speechSynthesis.cancel(); startFlow(); }}
            ...
            style={{ fontFamily: "Neuton, serif" }}>
            <Search size={20} /> {getMessage('researchMode', 'searchNewTopic', language)}
          </button>
          ...
          <div className="bg-[#1a1a1a] border border-gray-700 rounded-xl p-4">
            <p className="text-[#FFD700] font-bold mb-2 text-[14px]" style={{ fontFamily: "Neuton, serif" }}>
              {getMessage('researchMode', 'voiceCommandsHeader', language)}
            </p>
            <div className="grid grid-cols-2 gap-2 text-[13px] text-gray-300" style={{ fontFamily: "Inter, sans-serif" }}>
              <span>{getMessage('researchMode', 'commandReadPaperOne', language)}</span>
              <span>{getMessage('researchMode', 'commandReadPaperTwo', language)}</span>
              <span>{getMessage('researchMode', 'commandHighlights', language)}</span>
              <span>{getMessage('researchMode', 'commandSearchNewTopic', language)}</span>
              <span>{getMessage('researchMode', 'commandGoBack', language)}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  if (screen === "reading" && selectedPaper) {
    ...
    return (
      <div className="min-h-screen bg-black text-white px-6 py-8">
        <div className="max-w-4xl mx-auto flex flex-col gap-6">
          <div className="flex justify-between items-center">
            <h1 className="text-[48px] font-bold" style={{ fontFamily: "Neuton, serif" }}>{getMessage('researchMode', 'readingHeading', language)}</h1>
            <div className="flex gap-3">
              <button
                ...
                style={{ fontFamily: "Neuton, serif" }}>
                <ArrowLeft size={18} /> {getMessage('researchMode', 'papersButton', language)}
              </button>
              <button
                onClick={() => {
                  stopReadingRef.current = true;
                ...
                className="border-2 border-[#FFD700] text-[#FFD700] px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-[#FFD700] hover:text-black transition-colors"
                style={{ fontFamily: "Neuton, serif" }}>
                <Highlighter size={18} /> {getMessage('researchMode', 'highlightsButton', language)} ({highlights.length})
              </button>
            </div>
          </div>
          ...
          <div className="bg-[#1a1a1a] border border-[#FFD700] rounded-lg p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-[15px]" style={{ fontFamily: "Inter, sans-serif" }}>
                {isReading ? `${getMessage('researchMode', 'readingStatus', language, { section: currentSection, progress: readingProgress.toString() })}` : getMessage('researchMode', 'readyStatus', language)}
              </p>
              {transcript && <p className="text-gray-400 text-[12px]">{getMessage('researchMode', 'heard', language)} "{transcript}"</p>}
            </div>
          </div>
          ...
          <div className="bg-[#1a1a1a] border-2 border-[#FFD700] rounded-xl p-6">
            <p className="text-[#FFD700] text-[14px] mb-1" style={{ fontFamily: "Inter, sans-serif" }}>
              {getMessage('researchMode', 'paperDetails', language, {
          ...
          <div className="bg-[#1a1a1a] border border-gray-700 rounded-xl p-4">
            <p className="text-[#FFD700] font-bold mb-2 text-[14px]" style={{ fontFamily: "Neuton, serif" }}>
              {getMessage('researchMode', 'whileReadingSay', language)}
            </p>
            <div className="grid grid-cols-2 gap-2 text-[13px] text-gray-300" style={{ fontFamily: "Inter, sans-serif" }}>
              <span>{getMessage('researchMode', 'highlightThisLine', language)}</span>
              <span>{getMessage('researchMode', 'iLikeThisLine', language)}</span>
              <span>{getMessage('researchMode', 'stopReadingCommand', language)}</span>
              <span>{getMessage('researchMode', 'goBackCommand', language)}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-black text-white px-6 py-8">
      <div className="max-w-4xl mx-auto flex flex-col gap-6">
        <div className="flex justify-between items-center">
          <h1 className="text-[48px] font-bold" style={{ fontFamily: "Neuton, serif" }}>{getMessage('researchMode', 'highlightsButton', language)}</h1>
          <button onClick={() => { stopListening(); speechSynthesis.cancel(); onBack(); }}
            className="border-2 border-[#FFD700] text-[#FFD700] px-5 py-3 rounded-lg flex items-center gap-2 hover:bg-[#FFD700] hover:text-black transition-colors"
            style={{ fontFamily: "Neuton, serif" }}>
            <ArrowLeft size={20} /> {getMessage('researchMode', 'back', language)}
        ...
        {highlights.length === 0 ? (
          <div className="bg-[#1a1a1a] rounded-xl p-8 border border-gray-700 text-center">
            ...
            <p className="text-gray-400 text-[20px]">{getMessage('researchMode', 'noHighlightsSaved', language)}</p>
            <p className="text-gray-600 text-[14px] mt-2">{getMessage('researchMode', 'howToSaveHighlights', language)}</p>
          </div>
        ) : (
              ...
              <div key={h.id} className="bg-[#1a1a1a] rounded-xl p-6 border border-[#FFD700]">
                <div className="flex items-start justify-between mb-2">
                  <span className="text-[#FFD700] font-bold text-[15px]" style={{ fontFamily: "Neuton, serif" }}>
                    {getMessage('researchMode', 'highlightHeading', language, {
                      index: (i + 1).toString(),
                      section: getMessage(
                        'researchMode',
                        `section${h.section.charAt(0).toUpperCase() + h.section.slice(1)}`,
                        language,
                      ),
                    })}
                  </span>
                ...
                <button
                  ...
                  style={{ fontFamily: "Neuton, serif" }}>
                  <Play size={16} /> {getMessage('researchMode', 'playHighlight', language)}
                </button>
              </div>
            ))}
          </div>
        ...
        <button onClick={() => { stopListening(); speechSynthesis.cancel(); startFlow(); }}
          ...
          style={{ fontFamily: "Neuton, serif" }}>
          <Search size={22} /> {getMessage('researchMode', 'searchNewTopic', language)}
        </button>

        <button onClick={() => { stopListening(); speechSynthesis.cancel(); onBack(); }}
          className="w-full min-h-[60px] border-2 border-[#FFD700] text-[#FFD700] rounded-xl flex items-center justify-center gap-3 font-bold text-[18px] hover:bg-[#FFD700] hover:text-black transition-colors"
          style={{ fontFamily: "Neuton, serif" }}>
          <ArrowLeft size={20} /> {getMessage('researchMode', 'goBackToModeSelectionButton', language)}
        </button>
      </div>
    </div>
  );
...
async function listenOnce(ms: number, language: string): Promise<string> {
  const SR = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
  if (!SR) return recordAudio(ms, language);
...
async function askYesNo(
  ...
  const listen = listenOnceFn || ((ms: number) => listenOnce(ms, language));
...
const affirmative = /\b(yes|yeah|yep|sure|okay|ok|yup|yah|affirmative|correct|right)\b|అవును|సరే|ఓకే|ఓక/i;
    const negative = /\b(no|nah|nope|not really|negative|never)\b|కాదు|లేను|అవసరం లేదు/i;
  ...
  while (Date.now() - start < totalTimeout) {
    ...
    // unclear answer
    quickSpeak(getMessage('researchMode', 'pleaseSayYesNo', language));
  }
