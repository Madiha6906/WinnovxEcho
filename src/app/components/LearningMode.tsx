...
import { useLanguage, getMessage } from "../../services/language";
...
export default function LearningMode({ onBack }: LearningModeProps) {
  ...
  const MODE_LABELS = {
    main:        getMessage('learningMode', 'title', language),
    explanation: getMessage('learningMode', 'explanation', language),
    recall:      getMessage('learningMode', 'activeRecall', language),
    teach:       getMessage('learningMode', 'teachMeBack', language),
  };
  const {
    subMode,
    status,
    ...
    return (
      <div className="bg-[#1a1a1a] border-2 border-[#FFD700] rounded-xl p-6 flex flex-col gap-4">
        <p className="text-[20px] text-white/70" style={{ fontFamily: "Neuton, serif" }}>
          {getMessage('learningMode', 'sayModeOptions', language)}
        </p>
        {[
          { label: getMessage('learningMode', 'explanation', language), desc: getMessage('learningMode', 'learnDesc', language) },
          { label: getMessage('learningMode', 'activeRecall', language), desc: getMessage('learningMode', 'recallDesc', language) },
          { label: getMessage('learningMode', 'teachMeBack', language), desc: getMessage('learningMode', 'teachDesc', language) },
        ].map(({ label, desc }) => (
  ...
  if (subMode === "explanation") {
    return (
      <div className="flex flex-col gap-4">
        ...
        {!explanationState.topic && (
          <Prompt text={getMessage('learningMode', 'sayTopicAgain', language)} active={status === "listening"} />
        )}
      </div>
    );
  }
