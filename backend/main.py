...
async def transcribe(file: UploadFile = File(...), lang: str | None = None):
    ...
    segments, _ = model.transcribe(temp_path, language=language_code)
...
async def tts(request: TtsRequest):
    ...
    language_code = request.lang.split("-")[0].lower() if request.lang else "en"
    tts_lang = "te" if language_code == "te" else "en"

    try:
        tts_audio = gTTS(text=text, lang=tts_lang, slow=False)
